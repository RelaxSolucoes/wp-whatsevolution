...
(function($) {
    ...
    function init() {
        // Debug
        if (typeof wpwevoCheckout === 'undefined' || !wpwevoCheckout.debug) {
        ...
        var observer = new MutationObserver(function(mutations) {
            ...
            mutationTimer = setTimeout(function() {
                ...
                if (hasNewPhoneFields) {
                    if (wpwevoCheckout.debug) {
                        console.log('New phone fields detected, re-initializing validation');
                    }
                    isInitialized = false; // Permite a reinicialização
                    initializeValidation();
                }
            }, 500);
        });
        ...
        var checkoutBody = document.querySelector('.woocommerce-checkout');
        if (checkoutBody) {
            observer.observe(checkoutBody, {
                childList: true,
                subtree: true
            });
        }
